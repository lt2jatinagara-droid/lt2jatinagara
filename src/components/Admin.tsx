...
export default function Admin() {
  ...
  const setSanitizedData = (newData: any, referenceData: any = null) => {
    ...
    const merged = {
      ...
      recap_sd: ensure64TeamsForSd(mergeRecapLists(newData.recap_sd, ref.recap_sd || rawFallbackData.recap_sd), 23),
      recap_smp: ensure32Teams(mergeRecapLists(newData.recap_smp, ref.recap_smp || rawFallbackData.recap_smp), 31),
            ...
            <div className="flex gap-2 p-1 bg-brand-surface rounded-2xl border border-brand-border shrink-0 self-stretch sm:self-auto justify-center">
              ...
              >
                ...
                <span className={`px-2 py-0.5 rounded-full text-[10px] ${
                  ...
                  {(data?.recap_smp || []).filter((item: any) => item.team.toLowerCase().includes("putra") || item.tent_no.toUpperCase().startsWith("C.") || item.tent_no.toUpperCase().startsWith("PA") || item.tent_no.toUpperCase().startsWith("A")).length}
                </span>
              </button>
              <button
                type="button"
